...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (body.action === 'delete') {
      ...
      const { error } = await adminSupabase.from('articles_cms').delete().eq('id', body.id);
    ...
    if (body.action === 'update') {
      if (!body.id) return new Response(JSON.stringify({ error: 'Missing id' }), { status: 400 });
      const { error } = await adminSupabase.from('articles_cms').update(body.payload).eq('id', body.id);
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500 });
      return new Response(JSON.stringify({ ok: true }), { status: 200 });
    }

    if (body.action === 'insert') {
      const { error } = await adminSupabase.from('articles_cms').insert(body.payload);
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500 });
      return new Response(JSON.stringify({ ok: true }), { status: 200 });
    }
    ...
    console.error('[admin-articles-cms] threw:', err instanceof Error ? err.message : String(err), err instanceof Error ? err.stack : '');
