...
| Resend audience sync + backfill user_id | **Actionable now (unblocked by domain verification).** Sync `email_subscribers` to a Resend audience for sending, and backfill `user_id` on rows that were recorded by email only (email-confirmation signups and homepage-form rows that later created an account — see the `signup_checkbox` collision note in "email_subscribers model" Resolved). (The Google-OAuth opt-in gap is now handled — signup page persists checkbox intent across the redirect and `/auth/callback` replays it; those OAuth rows already carry `user_id` via the token, so backfill mainly covers email-confirmation and homepage-form rows.) |
