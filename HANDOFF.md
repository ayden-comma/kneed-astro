...
| Resend audience sync + backfill user_id | **Actionable now (unblocked by domain verification).** Sync `email_subscribers` to a Resend audience for sending, and backfill `user_id` on rows that were recorded by email only (email-confirmation signups and homepage-form rows that later created an account — see the `signup_checkbox` collision note in "email_subscribers model" Resolved). Also covers the deferred Google-OAuth opt-in gap (persist checkbox intent across the OAuth redirect). |
